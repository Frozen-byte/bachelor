...
Bei der neuen Anwendung soll es sich um ein Projekt handeln, welches sämtliche Vorteile existierender Programme vereint. Zudem vermeidet es die negativen Punkte der oben erwähnten Software. Die Anwendung legt viel Wert darauf erweiterbar zu sein, ein möglichst großes Spektrum an Aufgaben zu bieten und dynamisch Aufgaben zu generieren. \\
Dabei ist es wichtig, dass die Anwendung nicht zu komplex wird. Sie sollte für einen neuen Nutzer so intuitiv wie möglich verständlich sein. Zusätzlich sollte die Installation sehr einfach von der Bedienung sein. Im Zentrum steht die Idee des "Herunterladen und direkt starten". \\
Gleichzeitig wird es sich bei der resultierenden Webapplikation um Open Source handeln, das heißt der Quellcode ist frei verfügbar und kostenlos, ebenso wie die Webapplikation selbst. Dies bietet jeder Person, die sich mit den verwendeten Technologien auskennt, die Möglichkeit die Applikation um zusätzliche Features zu erweitern.
...
Als grobe Eingrenzung lässt sich sagen, dass diese Anwendung für die Lehre geschaffen wird, um genau zu sein für die Fächer Mathematik, Informatik und Physik. Die Applikation lässt sich nach dem momentanen Konzept nur während der Vorlesung / des Unterrichts benutzen. Dies ist in dem Konzept begründet, welches Teams vorsieht, die zusammenarbeiten und so Punkte sammeln. Die Arbeit als einzelne Person ist nicht vorgesehen. \\
Da die Anwendung voraussetzt, dass jeder Nutzer einen eigenen Zugang zum Internet hat, verlagert sich das Hauptnutzungsfeld in höhere Bildungsstufen, zum Beispiel Universitäten und Fachhochschulen. \\
Zusammenfassend ist die Anwendung hauptsächlich im Bereich der Lehre der Mathematik/Informatik in Universitäten und Fachhochschulen vorgesehen, wobei es durchaus denkbar ist, dass auch andere Fächer davon profitieren können.
...
Für das Projekt ist nun bereits klar, dass es sich um eine Webanwendung handelt. Allerdings stehen noch einige Fragen offen, wie zum Beispiel welche Technologie zum Ausliefern der Backend-Daten genutzt wird. Daher wird im kommenden Kapitel auf die genutzten Technologien eingegangen.

\subsection{MariaDB}


Bereits am Anfang der Entwicklung stand fest, dass viele Daten sowie die Accounts von Administratoren über mehrere Sitzungen hinweg persistiert werden müssen. Zusätzlich musste eine Möglichkeit geschaffen werden die erstellten Aufgaben-Generatoren zu speichern. Bei Bedarf müssen Daten abgerufen werden können. Daher war recht früh klar, dass eine Datenbank eingesetzt werden muss. \\
...
Bei relationalen Datenbanken liegt einer der Hauptvorteile in ihrer Struktur. Ihr Aufbau in mehrere Tabellen erlaubt sehr leicht Duplikate von Daten zu vermeiden (auch bekannt unter dem Begriff ``Normalisierung''). Dadurch bleibt die Datenbank kleiner und leichter zu durchsuchen, was für eine bessere Performance sorgt. \\

Sollte man allerdings Flexibilität benötigen, so sind Dokument-basierte Datenbanken die richtige Wahl. Bei diesen werden die Daten in einem Objekt - das Dokument - abgespeichert. Es besitzt weder Form noch Schema. Dadurch ist es möglich, dass die gespeicherten Daten sehr voneinander abweichen können. Am ehesten vergleichbar ist dies mit einem normalen JSON-Objekt. Da die Daten hier nicht über mehrere Tabellen verteilt sind, lässt sich ein Dokument jederzeit mit einem einzelnen Befehl (Query) aus der Datenbank extrahieren. Dokument-basierte Datenbanken sind immer die richtige Wahl, wenn viele Datensätze existieren, welche allerdings keine Beziehung zu einander stehen (nicht-relationale Datensätze). \\
...
Bei der Suche nach einer passenden Datenbank-Engine sind zwei in Frage gekommen: MariaDB oder MySQL. Bei MySQL handelt es sich um eine der bekanntesten Datenbank-Engines, die in der heutigen Welt existieren. 1994 gegründet und 2008 von Sun Microsystems übernommen, kann dieses Projekt auf eine lange Geschichte und viel Erfahrung zurückblicken, was den Standpunkt als Platzhirsch unter den Datenbanken nur festigt. \\
2009 ist aus dem MySQL-Projekt allerdings ein Neues entstanden. Einige der Entwickler haben sich von dem alten Team getrennt und ein Neues gegründet. Aus dieser Zusammenarbeit ist ein neues Produkt entstanden: MariaDB. Der Fork von MySQL hat inzwischen einige Vorteile gegenüber der alten Lösung und erfreut sich an wachsender Popularität. Dies liegt zum einen daran, dass MariaDB OpenSource ist, andererseits darin, dass MariaDB in den meisten Use-Cases performanter ist als MySQL. Zusätzlich bietet es Verschlüsselung auf Datenbank-Ebene an. In einem großen Spektrum an Speicher-Engines, die für verschiedene Zwecke optimiert wurden, können Daten schnell gespeichert werden. \\

Alles in allem lässt sich zusammenfassen, dass es sich bei MariaDB um die `smarte' Variante handelt, an der erheblich mehr gearbeitet wird. Daher ist die Wahl auf MariaDB gefallen. Da beide Datenbanken über die gleiche Sprache ansprechbar sind, ist es allerdings kein Problem jederzeit die Datenbank hinter OpenTasks auszutauschen.

\subsection{H2}

Ein wichtiger Punkt der Anwendung war allerdings auch, dass der Installationsprozess der Anwendung möglichst unkompliziert sein sollte. Dies ist leider nicht der Fall, wenn es notwendig ist, eine Datenbank zu installieren und initialisieren. OpenTasks bietet hier eine Alternative: Durch eine kleine Änderung der Konfiguration ist es möglich eine ``in-memory-database''(IMDB) zu benutzen. Dabei handelt es sich um eine Datenbank, die lediglich im Arbeitsspeicher der Maschine existiert und fast keine langsamen Lese- und Schreibzugriffe auf die Festplatte ausführt. Der Installationsaufwand wird dadurch erheblich reduziert. \\

Einer der Hauptvorteile einer solchen Datenbank liegt in der bessseren Zugriffszeit. Lese- und Schreib-Zyklen vom Arbeitsspeicher sind erheblich schneller als bei einer Festplatte. Allerdings hat eine IMDB nicht nur Vorteile: Arbeitsspeicher ist erheblich teurer als regulärer Festplattenspeicher, diese Kosten sollten nicht außer Acht gelassen werden. Da es sich bei OpenTasks nur um kleine Datenmengen handelt, ist dies in dieser Anwendung jedoch zu vernachlässigen. \\
...
Daher für eine dauerhafte Installation, eine Festplatten-basierte Datenbank empfohlen. Die Daten können so sicherer gespeichert werden. Für eine kurze Testphase ist IMDB allerdings eine angenehme Alternative.
...
Bei der Dependency-Injection handelt es sich um eine Technik, welches aus der heutigen Welt der Programmierung kaum wegzudenken ist. Einzelne Komponenten müssen ihre Abhängigkeiten nicht mehr selber erzeugen. Sie geben lediglich an, dass sie eine Abhängigkeit benötigen, welche einen bestimmten Satz an Aufgaben erfüllen kann (oft durch ein Interface definiert). Daraufhin sucht der Dependency-Injector nach einer passenden Komponente (in den meisten Frameworks als ``Service'' bezeichnet), die diese Anforderungen erfüllt, und fügt sie in den Anfragenden ein (bei diesem Prozess handelt es sich dann um die ``Injection''). \\
...
Die Technik der Dependency-Injection wird von den folgenden Frameworks Spring(-Boot) sowie Angular 2 implementiert und intensiv genutzt.
...
Schaut man sich als Letztes das Spring-Framework an, stößt man auf ein System, welches über ein inzwischen gigantisches Ökosystem an Erweiterungen verfügt. Seit März 2004 hat es immer mehr an Popularität und an Features dazu gewonnen. Einer der vielen Vorteile die Spring mitbrachte, war die Dependency-Injection, welche von Haus aus implementiert ist und direkt verwendet werden kann. Die Absicherung einer Anwendung wird durch Spring-Security ebenfalls erheblich vereinfacht. Es bietet direkten Schutz gegen einige Angriffe. Viele Authentifizierungsverfahren werden erheblich vereinfacht in der Benutzung. Eines der Probleme ist allerdings, dass es sich bei Spring (und all seinen Erweiterungen) inzwischen um ein immens großes Projekt handelt, was es sehr schwer machen kann mit Spring anzufangen und die erste Webanwendung zu schreiben. \\
...
Kurz gesagt hilft das Framework dabei ein Projekt modular aufzubauen, was insbesondere bei großen Anwendungen sehr hilfreich ist, da diese übersichtlich bleiben. Drei Kernkonzepte, die dabei besonders wichtig sind: Die Controller, die bereits erklärten Services und die Dependency-Injection. \\
...
Gleichzeit stellt Spring-Boot auch direkt einen Container für die Webapplikation zur Verfügung. Das bedeutet, dass die Programmierung sich komplett mit der REST-API und ihren Funktionalitäten beschäftigen kann und nur sehr wenig Zeit für die Konfiguration des Web-Servers aufgewendet werden muss.

Sollte eine Konfiguration nötig sein, so findet diese zu großen Teilen außerhalb des Codes statt. In dem Projekt kann eine Datei mit dem Namen ``application.properties'' hinterlegt werden. Hier können dann diverse eingestellt vorgenommen werden, wie zum Beispiel der Port des Servers.\\
...
Nach dem JPA nun die Verbindung mit der Datenbank aufbauen kann, muss definiert werden, wie die Datenbank-Entitäten in Java-Objekte umgewandelt werden und in welchen Tabellen sie sich befinden. In JPA wird dies durch Entitäten und deren angelegten Klassen erreicht. Annotationen legen fest, wie die Daten in das Objekt geladen werden. \\
...
Besonders für Entwickler, welche aus der Welt der stark-typisierten Programmierwelt kamen, war TypeScript ein großer Fortschritt. Durch die Möglichkeit Variablen stark zu typisieren, entstand die Möglichkeit Schnittstellen klarer zu definieren. Gleichzeitig verbesserten sich die bestehenden Auto-Vervollständigungs-Möglichkeiten der Entwicklungsumgebungen. \\
Die Vorteile, die sich daraus für Programmierer ergeben sind recht eindeutig: Eine Beschleunigung der Entwicklung, während die Testbarkeit vereinfacht wird und Fehler besser vermieden werden können. Kurz gesagt: Bessere Software in weniger Zeit. \\

Für die Anwendung war früh klar, dass es sich um eine Single Page Application (kurz SPA) handeln soll. Bevor auf das genutzte Framework eingegangen wird, werfen daher einen genaueren Blick auf die Vorteile einer SPA. \\
...
Nachdem nun evaluiert wurde, dass eine SPA ein Gewinn ist nun also die Frage nach dem Framework / der Library um die Anwendung zu programmieren. Im Raum stehen zwei Varianten: Zum einen ReactJS (welches seit längerem existiert und weit bekannt ist) oder das jüngere Angular2. \\
...
Zusätzlich implementiert Angular 2 eine bessere Trennung der verschiedenen Komponenten. Während in ReactJS HTML und JavaScript Code in der selben Klasse stehen, werden in Angular 2 klare Mechanismen aufgezeigt, wie diese sauber zu trennen und in verschiedenen Dateien abzulegen sind. Was gleichzeitig auch einer der Nachteile von Angular 2 ist. Dadurch, dass HTML und Script sauber getrennt werden, ist es schwerer die Script-Variablen im HTML zu referenzieren und alle Namen im Kopf zu behalten ohne die zweite Datei geöffnet zu haben. Mit zwei Bildschirmen wird dieses Problem jedoch an den meisten Arbeitsplätzen elegant umgangen. \\

Bei Angular 2, sowie ReactJS handelt es sich standardmäßig um Technologien, bei denen ``Client-Side Rendering'' betrieben wird. Der Server liefert alle Dateien aus, die benötigt werden. Der Client nimmt diese entgegen, rendert die einzelnen Komponenten und fügt diese korrekt zusammen. Die Last des Rendering wird auf den Client übertragen. Dadurch entsteht weniger Aufwand für den Server, was gleichzeitig für eine bessere Performance sorgt (da dieser nicht selber rendern muss). Was bei Angular 2 aber auch unterstützt wird, ist die Möglichkeit auf der Seite des Server zu rendern. Wenn man die Ladezeiten auf dem Client reduzieren will und einen performanten Server zur Hand hat, ist dies eine gute Lösung. Möglich ist dies durch die kürzlich heraus gekommene Bibliothek Angular Universal \footnote{https://universal.angular.io/}. \\
...
Dies funktioniert, indem auf der Webseite innerhalb eines HTML-Elements ein Text eingetragen wird, welcher einer Auswahl von mehreren Syntaxen folgt. Daraufhin wird der Bibliothek mitgeteilt, dass es die Seite scannen soll. Sollte sie Elemente finden, bei denen es sich um mathematische Formeln handelt, werden diese in HTML-Elemente umgewandelt, und korrekt formatierte Formel dargestellt. \\

Innerhalb von OpenTasks wird ausschließlich die LaTex-Syntax \footnote{https://en.wikibooks.org/wiki/LaTeX/Mathematics} für mathematische Formeln verwendet.

\chapter{Die grafische Darstellung der Anwendung - Das Frontend}

Das Frontend ist im Wesentlichen in zwei große Abschnitte geteilt: Die Ansicht des Nutzers (also die Anwendung, wie sie von den Studenten gesehen wird) und die Ansicht des Administrators, welche hauptsächlich die Aufgabe hat die Anwendung zu verwalten und Einstellungen zu ändern. Beiden Sichten werden im Nachfolgenden getrennt voneinander behandelt.
...
Nach dem Erhalt einer Aufgabe beginnt die Bearbeitung. Der erhaltene Inhalt wird als mathematische Formel dargestellt. Hierbei hilft die Bibliothek MathJax dabei den erhaltenen Text optisch ansprechend und mathematisch korrekt zu formatieren. Der Student hat nun die Zeit, die er braucht um die Aufgabe zu bearbeiten. Sollte er sich der Lösung sicher sein, gibt er diese über das entsprechende Formular ein und bestätigt das Ganze mit einem Druck auf den ``Senden'' Knopf. Die Eingabe wird an den Server weitergeleitet, welcher diese validiert. Für den Studenten gibt es nun drei Fälle: \\
\begin{enumerate}
\itemsep0em
...
\item Die Aufgabe wurde korrekt gelöst. Der Nutzer erhält eine neue Aufgabe, sowie einen Hinweis über die Korrektheit der Lösung.
...
Bei dem Zweiten handelt es sich um die Eingabeformulare. Je nachdem was für eine Aufgabe bearbeitet wird, wird ein Formular angezeigt, welches darauf ausgelegt ist die Lösung einfach einzugeben. Eine Sache, die alle gemeinsam haben, ist die Validierung, ob die momentane Antwort des Nutzers korrekt sein könnte und die graphische Darstellung davon (grün / rot- Färbung des Antwort Textes). Wie genau diese Validierung aussieht und welche weiteren Funktionalitäten das Formular bietet, ist allerdings von Aufgabe zu Aufgabe unterschiedlich.
...
Insgesamt existieren fünf Eingabeformulare für Aufgaben:
\begin{enumerate}
\itemsep0em
...
In diesem Kapitel werden diese genauer analysiert, ihre graphische Darstellung gezeigt, sowie die Validierung der Antwort erläutert. \\
...
Das angezeigte Formular wird immer von dem Administrator der Anwendung ausgewählt. Für weitere Informationen hierzu im Kapitel ``\ref{EditGenerator} Bearbeitung der Generatoren und ihrer Hilfsmittel''.
...
Daher hat der Administrator beim einloggen erheblich weniger Optionen zur Verfügung
\begin{figure}[htp]     % h=here, t=top, b=bottom, p=page
\centering
...
Die während der Laufzeit am wahrscheinlich meisten genutzte Seite ist die Übersicht. Hier wird eine Oberfläche dargestellt, welche optisch aufbereitet die Daten anzeigt, die von der Anwendung erzeugt werden: Die Zeit, wie viele Sekunden eine Aufgabe noch läuft sowie den aktuellen Stand an richtig / falsch gelösten Aufgaben der verschiedenen Teams \\
\begin{figure}[htp]     % h=here, t=top, b=bottom, p=page
\centering
...
Die erste Ansicht wird gezeigt, wenn aktuell kein Generator am Laufen ist. \\
...
Welcher Generator gestartet werden soll wurde durch ein Drop-Down realisiert. Die Namen aller erstellten Generatoren werden vom Server abgefragt und dann für die Befüllung der einzelnen Optionen genutzt.\\
Sollte der Startknopf gedrückt werden ohne eine ausgewählte Option, so erscheint eine Benachrichtigung für den Nutzer eine korrekte Auswahl zu treffen. Selbiges geschieht bei einer ungültigen Angabe für die Laufzeit des Generators. \\
...
Sollte aber bereits ein Generator gestartet sein, wird die andere Sicht gezeigt. Der Nutzer wird darüber informiert, dass bereits Aufgaben generiert werden. Zusätzlich wird der Name des momentan laufenden Generators angezeigt.\\
...
Das Programm wurde von Anfang an für mehrere Aufgaben-Generatoren ausgelegt. Daher wurden die Helper-Funktionen eingeführt. Bei diesen handelt es sich um Funktionen, welche erstellt werden und dann global verfügbar sind. Dadurch wird die Menge an überflüssigem Code, der in jedem Generator erzeugt wird, reduziert.
...
Sobald der Transpilierungs-Prozess abgeschlossen ist, wird die gerade erstellte Funktion mit den eingegeben Argumenten aufgerufen. Das Ergebnis wird daraufhin auf der Seite angezeigt. Der Administrator sollte sich nun vergewissern, dass es die Ausgabe den Erwartungen entspricht. Wenn das Skript einen zufälligen Faktor besitzt, ist es möglich den Test mehrere Male laufen zu lassen. Die genaue Anzahl kann in dem Feld ``TestCount'' angegeben werden. \\

Wenn nun alle Tests den Erwartungen entsprechen und das Ergebnis zufrieden stellend ist, besteht die letzte Aufgabe darin, es zu speichern. Dafür wurde ein Knopf neben dem Test-Knopf eingefügt, welcher die Anfrage an den REST-Server stellt. \\

\subsection{Bearbeitung der Aufgaben-Generatoren}

Nachdem die Helper-Funktionen bearbeitet und erstellt wurden. Kann man sich nun dem Hauptpunkt zuwenden: Den Aufgaben-Generatoren. Die Bearbeitung wird über den Tab ``Generator bearbeiten'' gestartet. \\
...
Das Variablen-Skript erzeugt Variablen die im aktuellen Skript genutzt werden können. Sie sind in der Lage sind zufällig Werte zu generieren und gewährleistet, dass verschiedene Aufgaben erzeugt werden können.Es besteht eine kleine die Chance, dass zwei generierte Aufgaben sich gleichen. Diese Chance sinkt jedoch je größer der Zahlenraum der generierten Werte ist. Dieses Skript muss ein JSON-Objekt zurück geben, welches alle Werte enthält die für die graphische Darstellung, sowie die Validierung der Lösung von Nöten sind. \\
Um die Erstellung von diesem Skript zu erleichtern wurde ein Transpiler-Feature eingebaut. Jeder Code, der von zwei Prozent-Zeichen eingeschlossen ist, wird zu einem Funktionsblock umgesetzt. Dies hilft dabei Code zu schreiben, bei dem Funktionen aufgerufen werden, die wiederum Funktionen als Parameter benötigen.\\
...
Als letztes Skript wird die Validierung der Antworten des Nutzers benötigt. Dieses Skript muss einen  Booleschen Wert zurück geben, welcher für die Korrektheit der übergebenen Antwort steht: `true' bei korrekter und `false' bei falscher Eingabe. \\
...
Diese können nun dafür genutzt werden, um die Nutzereingabe zu validieren. Für dieses Skript gelten dieselben Transpilierungs-Regeln wie für das Variablen-Skript.


\subsection{Das Teilen von Aufgaben}

Es ist nun also möglich Aufgaben-Generatoren sowie unterstützende Funktionalitäten zu erzeugen. Ein Feature welches allerdings fehlt ist das Importieren von Aufgaben.\\
Bisher war dies nur möglich, indem man Teile der Datenbank manuell kopiert. Dies konnte aber keine dauerhafte Lösung sein. Daher wurde ein Im- und Export von Entitäten eingeführt.
\subsubsection{Entitäten exportieren}
...
Will man eine Entität exportieren, muss man lediglich auf die zugehörige Checkbox oder dem Text neben ihr drücken. Sobald die Auswahl beendet ist und der Herunterladen Knopf betätigt wurde, werden alle ausgewählten Entitäten exportiert. Die enstehende Datei enthält alle eben gewählten Entitäten. Als Speicherformat wurde hierfür JSON gewählt, da dies nativ von JavaScript unterstützt wird und ohne Probleme gelesen und manipuliert werden kann. Diese Datei kann nun auf jedem gewünschten Weg verteilt und genutzt werden.


\subsubsection{Entitäten importieren}

Der Tab ``Entitäten hochladen'' dient dazu exportierte Entitäten wieder in das Programm zu laden und diese zu benutzen. \\
...
Andernfalls, wird der Prozess fortgesetzt. Die erkannten Entitäten werden mit den Entitäten die in der Datenbank existieren abgeglichen. Dies läuft in zwei Schritten ab:\\

1. Die imporiteren Entitäten werden mit existierenden in der Datenbank abgeglichen. Sollte eine gefunden werden, welche den selben Namen wie eine Datenbank-Entität besitzt wird diese vorgemerkt.\\

2. Die vorgemerkten Entitäten werden im Detail verglichen. Während vorher nur die Namen abgeglichen wurden, werden jetzt alle fraglichen Entitäten komplett aus der Datenbank geladen und an den Client geschickt. Dieser vergleicht nun die beiden Datensätze auf Gleichheit. Sollte ein Unterschied entdeckt werden, wird die Entität als ``Konflikt mit der Datenbank'' markiert. Ansonsten werden sie als ``äquivalent mit dem Datenbank-Eintrag'' markiert. \\
...
Die Ansicht ist in drei Zeilen aufgespalten: eine für die Generatoren, eine für die Variablen- und eine für die MathJax-Funktionen. Auf der linken Seite werden dem Nutzer alle Datensätze angezeigt, welche äquivalent in der Datenbank existieren und daher nicht hochgeladen werden müssen. Die Mitte dient der Anzeige aller Entitäten, die mit der Datenbank in Konflikt treten. Die rechte Seite dient der Anzeige der Entitäten, welche problemlos hochgeladen werden können. \\

Für die Anzeige dieser Seite gibt es drei Fälle:\\

\noindent \textbf{1. Es auscchließlich Einträge in der Datenbank gefunden, die bereits existieren. } \\
...
\noindent \textbf{3. Es wurde kein Konflikt festgestellt und die Datei enthält neue, unbekannte Entitäten.} \\
...
In der Implementierung wird so vorgegangen, dass immer eine Aufgabe erzeugt wird. Sollte dieser Prozess fehlschlagen, wird ein Fehler produziert. Dieser Fehler wird zum einen an den Studenten weitergeleitet und auf seiner Seite angezeigt. Zusätzlich wird jeder Fehler, der registriert wird, auf Server-Seite abgespeichert. Der Administrator hat nun die Möglichkeit in der Webanwendung den Tab ``Fehler-Log'' zu öffnen. Diese Seite sendet eine Anfrage an den Server. Dieser antwortet mit einem Datensatz, der alle Fehler enthält, die während der Laufzeit produziert wurden. Daraufhin können diese evaluiert und der entsprechende Generator korrigiert werden, sodass dieser Fehler in Zukunft vermieden werden kann. \\
...
Es stand fest, dass der Administrator in der Lage sein muss eigene Skripte zu schreiben. Die gewährleistet eine große Vielfalt der Aufgaben. Der Programmierer muss sich nicht mit der Erstellung von Generatoren beschäftigen, sondern kann sich auf die Web-Applikation selbst konzentrieren. \\
...
\begin{enumerate}
\itemsep0em
\item Die Sprache muss weit verbreitet sein. Es darf kein Zwang existieren, nur für OpenTask eine neue Sprache zu lernen.
\item Die Sprache sollte entweder Java selbst sein, oder muss innerhalb von Java ohne großen Aufwand ausgeführt werden können.
\end{enumerate}

Bei der Recherche hat sich gezeigt, dass JavaScript zu einer äußerst populären Sprache geworden ist. Da es sich bei JavaScript zusätzlich um eine Sprache mit sehr leichter Syntax handelt und sie sich zusätzlich sehr leicht in Java-Code integrieren lässt, ist die Wahl auf JavaScript gefallen. \\

Die Skripte selber werden aus den Skript-Entitäten erzeugt. Ein fertiges Skript besteht immer aus vier Funktionen: Die ersten drei sind vom Administrator geschrieben und für die Erzeugung der Variablen/des MathJax und der Validierung der Lösung zuständig. Als vierte Funktion existiert die Funktion, die im MathJax die Variablen einsetzt.\\
...
Eine der grundlegenden Fragen, wenn es um Sicherheit geht, ist die Authentifizierung. Für diese Anwendung wurden \hyperref[JWT]{JSON Web Tokens} gewählt um eine sichere Authentifizierung zu gewährleisten. \\
Bevor der erste Austausch von Nutzdaten erfolgen kann, muss zuerst der Austausch des Tokens implementiert werden: Von der Seite des Nutzers wird eine Anfrage gesendet, welche Nutzername und Passwort beinhaltet. Dieses wird mit der Datenbank abgeglichen. Wird der Nutzer gefunden und ist das Passwort korrekt, wird ein Token generiert und an den Anfrage-Steller zurück gesendet. Es sei gesagt, Passwörter nur in verschlüsselter Form in der Datenbank vorliegen. Diese Anwendung nutzt den Hash-Algorithmus MD5. Ein MD5 Hash ist nur unter großer Rechenleistung zu erraten und gewährleistet die sichere Aufbewahrung Sensibler Daten. Wenn ein ungwollter Lesezugriff auf die Datenbank auftreten sollte, so sind die Passwörter schwerer zu entschlüsseln. \\
Jedes mal wenn nun Daten abgefragt werden sollen, schickt der Nutzer sein Token mit. Der Server überprüft die Validität des Tokens, erst dann erhält der Nutzer die angefragten Daten. \\
...
Die REST-API von Open Tasks ist mit der Zeit gewachsen und besitzt im Endprodukt eine große Vielfalt an Möglichkeiten. Die komplette Dokumentation ist im Anhang einzusehen. Folgend wird die Art und Weise der Dokumentation beschrieben, damit sie im Anhang verständlich ist. \\

Die Dokumentation ist in Kapitel unterteilt: ein Controller, beziehungsweise eine Sub-Domäne. Jedes Kapitel beginnt mit dem Recht, welches benötigt wird, um darauf zuzugreifen. Die Dokumentation der Rollen und Rechte ist im Kapitel \ref{RechteSystem} ``Authentifizierung und Rollen-System'' zu finden. \\
...
In der Dokumentation der REST-Schnittstelle werden drei spezielle Begriffe verwendet: \\
...
Abschließend noch die Helper-Funktionen. Es handelt es sich um den Sammel-Begriff für MathJax und Funktions-Entitäten, welche sich in ihrer Signatur exakt gleichen. \\
\begin{description}
\itemsep0em
...
Für die Implementation wurden die in der Einleitung erklärten Websockets verwendet. Sobald der Administrator auf die Übersichts-Seite wechselt, sendet die Webapplikation automatisch eine Anfrage an den Server um eine Websocket-Verbindung zu öffnen. Der Server nimmt diese jede Anfrage an. \\
...
Kommen wir zu der Hauptpaufgabe der Anwendung: die Generierung und Validierung der Aufgaben. Jedes Mal, wenn ein Generator angefragt wird, sucht der Server eine passende Entität aus der Datenbank. Die hier geladene Entität enthält allerdings nur Bruchstücke des Codes zur Generierung, diese reichen jedoch aus um den Generator eindeutig zu rekonstruieren. \\
Sobald die Rekonstruktion abgeschlossen ist, wird eine virtuelle Machine(VM) gestartet. In dieser kann der Code ausgeführt und die Aufgaben generiert werden. Um die VM zu starten und die Kommunikation zwischen Java und JavaScript-Code zu ermöglichen, wird die Java Scripting API\footnote{https://docs.oracle.com/javase/8/docs/technotes/guides/scripting/prog\_guide/api.html} genutzt, auf die hier nicht weiter eingangen wird. \\

Sobald der Generator geladen wurde, wird eine Rückmeldung an die Routine gegeben die ihn gestartet hat. Kommt nun eine Anfrage eines Clients an den Server eine Aufgabe zu generieren, wird dies an die JavaScript-VM weitergeleitet. Der dafür benötigte Kontext wird hierbei der eindeutigen ID des Nutzers zugeordnet und abgespeichert. Bei erfolgreicher Generierung wird die generierte Aufgabe zurück gegeben, damit sie vom Nutzer bearbeitet werden kann. Sollte während der Generierung ein Fehler auftreten so wird dieser in den Fehler-Log geschrieben, damit er vom Administrator eingesehen werden kann. Der Nutzer bekommt in dem Falle lediglich die Rückmeldung, dass während der Generierung seiner Aufgabe ein Fehler aufgetreten ist. Sollte es sein Wunsch sein, kann er daraufhin ein weiteres Mal eine Aufgabe anfordern. \\

Sobald der Student die Aufgabe bearbeitet, sendet der Client die eingegebene Lösung zum Server. Es wird als Erstes überprüft ob überhaupt ein Generator aktiv ist. Sollte keiner aktiv sein wird dem Nutzer gemeldet, dass die Bearbeitungszeit bereits abgelaufen ist. Ist dies nicht der Fall, wird die eindeutige ID des Nutzers aus dem Token, welches er zur Authentifizierung genutzt hat, ausgelesen. Diese ID wird nun genutzt um den Kontext der Aufgabe zu finden. Hier bestehen nun zwei Fälle: \\

1. Es wurde kein Kontext für die ID des Nutzers gefunden.\\
Dies ist immer dann der Fall, wenn seit dem Erhalt der Aufgabe ein neuer Generator gestartet wurde. Sobald ein neuer Generator gestartet wird, wird jedes Mal die Liste von allen momentan gespeicherten Kontexten (die Variablen, welche bei der Generierung genutzt wurden) gelöscht und eine neue angelegt. \\

2. Der Kontext wurde gefunden.\\
In diesem Fall kann mit der Validierung der Aufgabe begonnen werden. Der JavaScript-VM wird der Kontext, sowie die Antwort des Nutzers übergeben. Diese validiert nun die Antwort und gibt entsprechend `true' oder `false' an den Java-Teil der Anwendung zurück. \\
...
Sollte die Antwort keine gültige Lösung sein, wird keine neue Aufgabe generiert. Stattdessen wird dem Team des Nutzers eine falsch gelöste Aufgabe zugeschrieben (und alle verbundenen Administratoren informiert). An den Nutzer kommt lediglich die Rückmeldung, dass die Antwort falsch war.
...
Rückblickend lassen sich mehrere Erkenntnisse aus der Entwicklung und dem Produkt ``Open Tasks'' ziehen. \\

Eine der größten Schwierigkeiten war es, schadhaftern Code in ein Programm zu finden, wenn dem Nutzer erlaubt wird eigene Skripte zu erstellen. Immer wieder sind große Hindernisse in den Weg gekommen oder große Lücken aufgefallen, durch die sich doch etwas einschleusen lässt. Nicht alle davon ließen sich ohne weiteres beheben. Daher ist es in der Endversion von Open Tasks so, dass die vom Administrator erzeugten Skripte nicht auf unerlaubte Funktionen überprüft werden. Die alte Funktionalität ist noch da, da sie allerdings der vollen Funktionalität von Open Tasks schadet, wurde sie zeitweise deaktiviert. \\
Damit besteht die einzige Sicherheit darin, dass nur der Administrator Zugriff auf die Editierung der Skripte erhält. \\

Ein weiterer großer Punkt war die Zerteilung des Programmes in mehrere logische Einheiten um die Komplexität zu senken. Durch die Zerteilung wurden triviale Programm-teile unnötig aufgebläht um die Kommunikation untereinander zu ermöglichen. Dies wirkte sich jedoch negativ auf die Komplexität aus, welche versucht wurde zu senken. Dies lag an vielen nicht eindeutig zuordenbaren Einheiten. So befanden sie sich teilweise in zwei verschiedenen Abschnitten des Programmes. \\
...
Jedes Programm lässt sich ewig erweitern und verbessern. Open Tasks bildet hier keine Ausnahme. Einige Sachen sind nicht optimal und könnten in einer weiteren Iteration der Entwicklung verbessert werden. \\

Einer der großen Punkte ist das optische Auftreten der Webapplikation. Es wurde bereits relativ viel Arbeit investiert, die Anwendung grafisch ansprechend zu gestalten, doch das Ergebnis ist nicht das Ende aller Möglichkeiten in diesem Bereich. \\
...
Neben den Änderungen am Interface gibt es allerdings auch mehrere Dinge, die im Code optimiert werden könnten und keinen direkt ersichtlichen Einfluss auf Open Tasks hätten.\\

Unter besteht bei der Test-Ebene viel Platz zur Optimierung. Momentan besitzen weder das Frontend noch das Backend von Open Tasks Tests. Angular 2 sowie Spring-Boot bieten ausgezeichnete Möglichkeiten Unit-Tests zu schreiben, auch für Integration-Tests gibt es gut dokumentierte Möglichkeiten. \\

Abschließend ist noch das Rollen-System zu nennen. Momentan sind die Rollen und zugehörigen Zugriffsrechte fest im Code definiert und nicht ohne Änderung des Codes erweiterbar. Sollte die Applikation wachsen, könnte sich dies zu einem Hindernis entwickeln. Daher steht es zur Option die Rechte, die jeder Rolle zustehen nicht im Code, sondern in der Datenbank zu definieren und dynamisch auszulesen.

Glücklicherweise ist Open Task eine Open Source Applikation und kann somit von jedem versierten Programmierer stetig weiter entwickelt werden.
